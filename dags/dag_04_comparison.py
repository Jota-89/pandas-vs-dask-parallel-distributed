...
def generate_comparison_report(**context):
    ...
    parallel_file = results_dir / "dask_simple_results.json"  # ARCHIVO DASK SIMPLE
    ...
    if not parallel_file.exists():
        raise Exception(
            "No hay resultados paralelos! Ejecuta dag_03_dask_simple primero")
...
def generate_technical_charts(**context):
    ...
    # Cargar datos
    sequential_file = results_dir / "sequential_results.json"
    parallel_file = results_dir / "dask_simple_results.json"
    ...
    print(f"Sistema detectado: {cpu_count} cores, {cpu_count_logical} threads, {memory_gb:.2f}GB RAM")
    
    # GRÁFICO 1: ANÁLISIS DE PERFORMANCE TÉCNICO
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('ANÁLISIS TÉCNICO DE PERFORMANCE - NYC TAXI BENCHMARK', fontsize=16, fontweight='bold')
    ...
    bars2 = ax2.bar(metrics, values, color=colors_metrics, alpha=0.8, edgecolor='black', linewidth=1)
    ...
    bars3 = ax3.bar(resource_labels, resource_values, color=colors_resources, alpha=0.8, edgecolor='black', linewidth=1)
    ...
    bars4 = ax4.bar(throughput_methods, throughput_values, color=['#e74c3c', '#27ae60'], alpha=0.8, edgecolor='black', linewidth=1)
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('ANÁLISIS DE ESCALABILIDAD Y RECURSOS', fontsize=16, fontweight='bold')
    
    # 2A. Escalabilidad teórica vs real
    workers = np.arange(1, cpu_count + 1)
    theoretical_speedup = workers
    real_speedup_projected = [1 + (speedup - 1) * (w / cpu_count) * (1 - 0.1 * (w - 1) / cpu_count) for w in workers]
    
    ax1.plot(workers, theoretical_speedup, 'b--', linewidth=2, label='Speedup Teórico', marker='o')
    ax1.plot(workers, real_speedup_projected, 'r-', linewidth=2, label='Speedup Real Proyectado', marker='s')
    ax1.axhline(y=speedup, color='green', linestyle='-', linewidth=3, label=f'Speedup Actual ({speedup:.2f}x)')
    ax1.axvline(x=cpu_count, color='orange', linestyle=':', linewidth=2, label=f'CPU Cores ({cpu_count})')
    ...
    efficiency_values = [s / w for s, w in zip(real_speedup_projected, workers)]
    
    ax2.bar(workers, efficiency_values, color='orange', alpha=0.7, edgecolor='black')
    ax2.axhline(y=efficiency, color='red', linestyle='-', linewidth=3, label=f'Eficiencia Actual ({efficiency:.2f})')
    ...
    overhead_percentage = [(theoretical_speedup[i] - real_speedup_projected[i]) / theoretical_speedup[i] * 100 for i in range(len(workers))]
    ...
    ax4.axhline(y=par_time, color='red', linestyle='-', linewidth=3, label=f'Tiempo Real ({par_time:.2f}s)')
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle('ANÁLISIS DE MEMORIA Y CPU - SISTEMA', fontsize=16, fontweight='bold')
    ...
    bars = ax1.bar(memory_methods, memory_usage, color=['#e74c3c', '#27ae60'], alpha=0.8, edgecolor='black')
    ...
    cpu_par = np.where(time_points <= par_time, 100 * cpu_count * 0.8, 0)  # Múltiples cores
    
    ax2.fill_between(time_points, 0, cpu_seq, alpha=0.7, color='red', label='CPU Secuencial')
    ax2.fill_between(time_points, 0, cpu_par, alpha=0.7, color='green', label='CPU Paralelo')
    ...
    bars = ax4.bar(energy_methods, energy_usage, color=['#e74c3c', '#27ae60'], alpha=0.8, edgecolor='black')
